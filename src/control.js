...
function initMonacoEditor(script) {
    ...
    if (!container || monacoEditors[script.id]) return;    
    ...
    // Manual clipboard and keyboard shortcuts for Electron
    editor.addAction({
        id: 'custom.paste',
        label: 'Paste',
...
function createScriptAPI(scriptId) {
    ...
    // Create a new text object
        ...
        updateTextObject: function(id, updates) {
            ...
            // If script is updating position/text, disable new animations 
    ...
    // Helper functions that use the api methods
    ...
    api.fadeTransition = function(id, newText, fadeOutDuration = 800, fadeInDuration = 800) {
        ...
        // Fade out
        api.fadeOut(id, fadeOutDuration);        
        // Change text in the middle
        api.setTimeout(() => {
            api.updateTextObject(id, { text: newText });            
            api.setTimeout(() => {
...
function deleteAnimationPreset(id, event) {
    ...
    if (index !== -1) {
        ...
        // Clear animation from any text objects using this preset
        document.getElementById(`animation-preset-${id}`).remove();      
...
function updateInputMode(id, mode) {
    ...
    if (obj) {
        ...
        if (obj.inputMode === 'file' && mode !== 'file') {
            ipcRenderer.send('stop-file-watch', id);
        }
...
ipcRenderer.on('project-loaded', (event, { projectData, filePath }) => {
    try {
        ...
        animationPresets = [];
        document.getElementById('animation-presets-list').innerHTML = '';  
        scripts.forEach(s => {
        ...
        if (projectData.globalSettings) {
            ...
            document.getElementById('bg-transparent').checked = projectData.globalSettings.backgroundTransparent;            
            ipcRenderer.send('update-background', globalSettings);
        }
        
        // Load animation objects first
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Always initialize click-through as disabled on startup
    document.getElementById('enable-click-through').checked = false;
    
    // Create default animation preset first
    createAnimationPreset();
...
function updateAccentColor(color) {
    ...
    });    // Add drag-to-change for number inputs 
